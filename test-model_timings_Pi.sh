...
echo dual-core
perf stat -o logs/test-model/$identification/dual-core_perf.txt -e cache-misses,cache-references,LLC-loads,LLC-load-misses,LLC-stores,LLC-store-misses taskset -c 0,1 python test-model.py 2 > logs/test-model/$identification/dual-core.txt &
wait
echo end dual-core
sleep 300
...
echo tri-core
perf stat -o logs/test-model/$identification/tri-core_perf.txt -e cache-misses,cache-references,LLC-loads,LLC-load-misses,LLC-stores,LLC-store-misses taskset -c 0,1,2 python test-model.py 3 > logs/test-model/$identification/tri-core.txt &
wait
echo end tri-core
sleep 300

echo quad-core
perf stat -o logs/test-model/$identification/quad-core_perf.txt -e cache-misses,cache-references,LLC-loads,LLC-load-misses,LLC-stores,LLC-store-misses taskset -c 0,1,2,3 python test-model.py 4 > logs/test-model/$identification/quad-core.txt &
wait
echo end quad-core
sleep 300
