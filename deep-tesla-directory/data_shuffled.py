...
# "runs/run{0}/out-mencoder.avi".format(epoch_id) #
# "runs/run%i/data.csv" % epoch_id #
...
def load_imgs():
    ...
    for p in purposes:
        for epoch_id in epochs[p]:
            print 'processing and loading "{}" dataset {} into memory, current num of imgs is {}...'.format(p, epoch_id, len(imgs[p]))
