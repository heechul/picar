...
%% There are already several relatively inexpensive RC-car
...
%% , which uses a significantly more
%% powerful computer (Drive PX computer~\cite{drivepx}) than a Raspberry
%% Pi~3.
%% We chose to use a Pi 3 not only because it is affordable, but 
%% also because it is representative of today's mainstream low-end
%% embedded multicore platforms found in smartphones and other embedded
%% devices.
...
%% is equipped with a NVIDIA GTX 1050 Ti GPU. Finally, the trained network is copied
...
Second, we find that real-time processing of CNN inferencing is
...
\begin{itemize}
  \item We present DeepPicar, a low-cost autonomous car testbed using
    ...
    cars.~\footnote{The source code, dataset, and build instructions
      of DeepPicar can be found at:\fixme{Michael github url}}  
