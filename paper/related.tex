...
\fixme{What we want to say:
  ...
  3) show table, borrowed from google's object detection api
  paper~\cite{huang2017speed}. 
  4) Applying these much more complex DNNs would improve network accuracy
  but they are obviously challenging for embedded computing
  platforms. We already discussed computing platform improvement that
  improve performance of executing a given DNN workload in
  section 2, so here we discuss model compression that reduces
  the necessary computing load itself.}
...
\fixme{discuss MIT racecar, UPENN car, donkey car and other testbeds?
