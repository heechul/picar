...
In order to determine if the DNN is mostly affected by shared DRAM 
...
as can be seen by \ref{fig:cache-mapping}. This results in 
...
the L1D cache is not partitioned in our experiments.
...
are seen, which further shows that the shared cache has little affect on 
...
decreases, depending on the circumstances. As a result, we conclude that 
