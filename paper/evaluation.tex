...
We use the SCHED\_FIFO real-time scheduler to schedule the DNN control
...
experimentally measured the execution times of DeepPicar's DNN
...
Figure~\ref{fig:perf_vs_bandwidth} shows the result. Note
that BwWrite co-runners cause significantly high execution time
incrases---up to 11.6X---on DNN inferencing while BwRead co-runners
cause relatively much smaller time increases. While execution time
increases are expected, the degree of which in the worst-case is quite
surprising---\emph{if the DNN controller was driving an actual car, it
  would result in a crash of the car!}
...
in multicore. Thus, in the following, we investigate if and how
