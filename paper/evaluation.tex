...
expected due to contention in the shared hardware
resources, such as the shared cache and DRAM.
...
\subsection{Effect of Co-scheduling Synthetic Benchmarks}\label{sec:eval-memhog}

In this experiment, we further investigate the impact of shared
resource contention to the DNN inference timing of DeepPicar using  
synthetic memory benchmarks. Specifically, we use the \emph{Bandwidth}
...
experiement first with BwRead~\footnote{\texttt{\$ bandwdith -a read
    -m 16384 -t 1000}} and next with BwWrite~\footnote{\texttt{\$
    bandwdith -a write -m 16384 -t 1000}}.
...
model against varying number of BwRead and BwWrite co-runners. First,
note that BwWrite cause significantly high execution time incrase---up
...
controller was driving an actual car, it would result in crash of the
car!}
