...
Raspberry Pi3's BCM2837 processor. We use the bits 13 and 14 for
coloring, which results in 4 page colors.
...
\begin{figure}[h]
  ...
  \caption{Cache space sensitivity of the control loop exectuion time.}
...
different cgroups which are configured to use 1, 2,
3, and 4 colors (25\%, 50\%, 75\%, and 100\% of the L2 cache
...
We also run a single model alongside three memory intensive read co-runners 
again, but ensure that each task is given equal amounts of L2 cache space.
For this, we assign one color to each task. The results can be seen in Figure
\ref{fig:palloc_bandwidth_exectime}. When cache partitioning is enabled, 
DNN performance becomes slightly better, with a improvement of 
\textasciitilde6.5\%. However, as the performance improvement is minimal, 
we do not believe that cache partitioning is effective for protecting the model 
from interference from other memory intensive processes.
