...
then converted as the PWM values of the steering motor of the
car. Figure~\ref{fig:controlloop} shows simplified pseudo code of the
controller's main loop. Among the five steps, step 3, network
inferencing, is the most computationally intensive and dominates the
execution time.
...
long depending on camera and the performance of the Pi. We
...
could negatively affect to the control performance, because the DNN would
