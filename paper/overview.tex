...
developing DeepPicar, one of our primary goals is to faithfully
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.7\textwidth]{figs/architecture}
...
NVIDIA DAVE-2's convolutional neural network (CNN) using TensorFlow,
based on ~\cite{deeptesla}. Note, however, that the CNN model
in~\cite{deeptesla} was considerably bigger than NVIDIA's CNN as it contained an
additional fully-connected layer of approximately 1.3M additional
parameters. We eliminated the additional layer to faithfully recreate
the NVIDIA's original CNN model.
...
used in this paper, which
is comprised of 8 layers, 250K parameters, and about 27 million
connections. It is identical to NVIDIA DAVE-2's CNN except that we do
not apply a normalization layer as it does not include trainable
parameters and its computational demand is minimal.
...
DeepPicar could remain on a track for over 10
minutes~\footnote{A collection of self-driving videos can be found at: \url{https://photos.app.goo.gl/ce93sU7jPk4ywO8u2}}.
