...
already, Raspberry Pi 3's increased L2 misses do not fully explain
increase in processing times. In case of Intel UP, as the number of
interfering Bandwidth benchmark instances increases, the L2 miss rates
even drop slightly. In case of TX2 (CPU), from solo to one co-runner,
L2 miss rate increase almost twice, but then the execution time
increase is only about less than 10\%. However, when there are
co-runners, even though the L2 miss rates are largely unchanged, the
DNN processing time increases significantly. TX2 (GPU) also shows
